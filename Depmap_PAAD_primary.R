...
class(data_crispr)
data_crispr_PAAD_Primary = data_crispr %>%
  dplyr::filter(ModelID %in% PAAD_Primary_sample_info$ModelID)

dim(data_crispr_PAAD_Primary)
write.csv(data_crispr_PAAD_Primary, "data_crispr_PAAD_Primary.csv")
df1 = data_crispr_PAAD_Primary[, 2:18436]
median(df1[, 217], na.rm = TRUE)

data_crispr_PAAD_median = rbind(df1, Medians = apply(df1, 2, median, na.rm = TRUE))
as.numeric(data_crispr_PAAD_median[26,])
data_crispr_PAAD_median_extract = data.frame("Gene" = colnames(data_crispr_PAAD_median),
                                             "Median" = as.numeric(data_crispr_PAAD_median[26,]))
...
write.csv(data_crispr_PAAD_median_extract, "data_crispr_PAAD_Primary_median_extract.csv") 
...
g <- g + xlab("PAAD_Primary Median CRISPR Score")
g <- g + ylab("Rank")
...
ggsave(plot=g, filename="./PAAD_Primary_Median_CRISPR_Score_gene_ranking.pdf", units = "in", width=8, height= 8, dpi=600)
...
class(data_mRNA)
data_mRNA_PAAD_Primary = data_mRNA %>%
  dplyr::filter(ModelID %in% PAAD_Primary_sample_info$ModelID)

dim(data_crispr_PAAD_Primary)
dim(data_mRNA_PAAD_Primary)
write.csv(data_mRNA_PAAD_Primary, "data_mRNA_PAAD_Primary.csv")

## match data_mRNA_PAAD with data_crispr_PAAD
data_mRNA_PAAD_Primary_overlap_crispr = data_mRNA_PAAD_Primary %>%
  dplyr::select(ModelID, colnames(data_crispr_PAAD_Primary)[colnames(data_crispr_PAAD_Primary) %in% colnames(data_mRNA_PAAD_Primary)])
dim(data_mRNA_PAAD_Primary_overlap_crispr)

data_mRNA_PAAD_Primary_overlap_crispr = data_mRNA_PAAD_Primary_overlap_crispr %>%
  dplyr::filter(ModelID %in% data_crispr_PAAD_Primary$ModelID)
dim(data_mRNA_PAAD_Primary_overlap_crispr)
dim(data_crispr_PAAD_Primary)

data_crispr_PAAD_Primary_overlap_mRNA = data_crispr_PAAD_Primary%>%
  dplyr::filter(ModelID %in% data_mRNA_PAAD_Primary_overlap_crispr$ModelID)
dim(data_crispr_PAAD_Primary_overlap_mRNA)

data_mRNA_PAAD_Primary_overlap_crispr = data_mRNA_PAAD_Primary_overlap_crispr %>%
  arrange(ModelID)

data_mRNA_PAAD_Primary_overlap_crispr_ID1 = data_mRNA_PAAD_Primary_overlap_crispr %>%
  dplyr::select(ModelID, ID1)

colnames(data_mRNA_PAAD_Primary_overlap_crispr_ID1)[2] = "ID1_mRNA"

data_crispr_PAAD_Primary_overlap_mRNA_ID1 = data_crispr_PAAD_Primary_overlap_mRNA %>%
  dplyr::select(ModelID, ID1)

colnames(data_crispr_PAAD_Primary_overlap_mRNA_ID1)[2] = "ID1_crispr"

corr_ID1_PAAD_Primary_mRNA_crispr = data_crispr_PAAD_Primary_overlap_mRNA_ID1  %>%
  left_join(data_mRNA_PAAD_Primary_overlap_crispr_ID1, by = "ModelID")

##cbind(data_crispr_PAAD_metastasis_overlap_mRNA_ID3$ModelID, data_mRNA_PAAD_metastasis_overlap_crispr_ID3$ModelID)

writexl::write_xlsx(as.data.frame(corr_ID1_PAAD_Primary_mRNA_crispr), "corr_ID1_PAAD_Primary_mRNA_crispr.xlsx")
...
corr_ID1_PAAD_mRNA_crispr = corr_ID1_PAAD_Primary_mRNA_crispr
...
ggsave(plot=p, filename="PAAD_metastasis_ID1_CRISPR_mRNA_Correlation.pdf", units = "in", width=12, height=8)  
