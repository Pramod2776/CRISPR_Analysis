...
write.csv(data_crispr_PAAD_median_extract, "data_RNAi_PAAD_median_extract.csv") 

data_crispr_PAAD_median_extract = na.omit(data_crispr_PAAD_median_extract)
dim(data_crispr_PAAD_median_extract)
...
Top5_pos = data_crispr_PAAD_median_extract$Gene[16811:16815]
...
g <- g + xlab("PAAD Median RNAi Score")
g <- g + ylab("Rank")
...
ggsave(plot=g, filename="./PAAD Median RNAi Score_gene_ranking.pdf", units = "in", width=8, height= 8, dpi=600)
...
colnames(data_crispr_PAAD_overlap_mRNA_ID1)[2] = "ID1_RNAi"

corr_ID1_PAAD_mRNA_crispr = data_crispr_PAAD_overlap_mRNA_ID1 %>%
  left_join(data_mRNA_PAAD_overlap_crispr_ID1, by = "ModelID")

corr_ID1_PAAD_mRNA_crispr = na.omit(corr_ID1_PAAD_mRNA_crispr)

cbind(data_crispr_PAAD_overlap_mRNA$ModelID, data_mRNA_PAAD_overlap_crispr$ModelID)

writexl::write_xlsx(as.data.frame(corr_ID1_PAAD_mRNA_crispr), "corr_ID1_PAAD_mRNA_RNAi.xlsx")
...
options(ggrepel.max.overlaps = Inf)
p = ggplot(corr_ID1_PAAD_mRNA_crispr, aes(ID1_RNAi, ID1_mRNA, label = ModelID)) +
  geom_text_repel(
    data          = ID1_crispr_map,
    force_pull   = 0, # do not pull toward data points
    nudge_y      = 0.05,
  ...
  xlab("PAAD_ID1_RNAi") +
  ylab("PAAD_ID1_mRNA") +
  ggtitle("Correlation of PAAD_ID1_RNAi and PAAD_ID1_mRNA")


ggsave(plot=p, filename="PAAD_ID1_RNAi_mRNA_Correlation.pdf", units = "in", width=12, height=8)  
...
pdf(file = "ID1_RNAi_complete_box_Plot_primary_cancer.pdf",   # The directory you want to save the file in
...
boxplot(score ~ new_order, data = df, col = "white",
        main = "ID1 DepMap RNAi score Cancer Status",
        ##xlab = "Cancer type Clinical STATUS",
        ylab = "ID1 DepMap RNAi score",
        yaxt = "n", las = 2,
        ylim = c(-1.25, 1))
axis(2, at= seq(-1.25, 1, 0.25))
mtext(side=1, text="Cancer type Clinical STATUS", line=9)
...
pdf(file = "ID3_RNAi_complete_box_Plot_primary_cancer.pdf",   # The directory you want to save the file in
    width = 15, # The width of the plot in inches
    height = 10)
par( mar=c(25,10, 2.3, 2))
# boxplot(score ~ cancer, data = df, col = "white",
...
boxplot(score ~ new_order, data = df, col = "white",
        main = "ID3 DepMap RNAi score Cancer Status",
        ##xlab = "Cancer type Clinical STATUS",
        ylab = "ID3 DepMap RNAi score",
        yaxt = "n", las = 2,
        ylim = c(-1.25, 1))
axis(2, at= seq(-1.25, 1, 0.25))
mtext(side=1, text="Cancer type Clinical STATUS", line=9)
